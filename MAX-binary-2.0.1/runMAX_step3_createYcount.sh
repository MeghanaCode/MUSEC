...
bash /path/to/constructMutXmatrix.sh -m $mutlist -g $gtffile -r $fastafile -v $hgversion -d $OUTPUT -t $CPUNUM -k $kmer
fi
...
# ## MAX2 -  Estimation
# # use AEM - default
# Rscript /path/to/R/estimateBeta_MAX2.R workdir=$Ycount_MAX2_outdir sampleMut=$sampleMutFn out=MAX2_isoform_expression.RData core=$CPUNUM
# if [ -d $Ycount_MAX2_outdir ]; then
