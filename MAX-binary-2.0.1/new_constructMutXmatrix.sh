...
Rscript /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/R/MAX.R mut=$mut gtf=$gtf ref=$ref workdir=$workdir hg=$hg
...
chmod +x /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/*

cd $workdir
mkdir WT_Mut_Ref
mv WT_Mut_reference.fa WT_Mut_Ref
# Index
/cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/TxIndexer -t WT_Mut_Ref/WT_Mut_reference.fa -o Index_reference -k $kmer -p $CPUNUM --force

## generate RNA-seq using polyester
mkdir xSimDat
#Rscript /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/R/genPolyesterSimulation.R WT_Mut_Ref/WT_Mut_reference.fa xSimDat
#/cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/GenTC -i Index_reference -l IU -1 xSimDat/sample_01_1.fasta -2 xSimDat/sample_01_2.fasta -p $CPUNUM -o $workdir

### generate RNA-seq using a fast tool
#/cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/simx_pair WT_Mut_Ref -o xSimDat -t $CPUNUM -d 2

nbatch=10
Rscript /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/R/splitFasta.R in=WT_Mut_Ref/WT_Mut_reference.fa k=$nbatch
for ((i=1; i <= $nbatch; i++))
do
    echo $i
    ...
    foldFn=$(echo "fold_"$i)
 #   mkdir $simFn
    /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/simx_pair $foldFn -o $simFn -t $CPUNUM -d 2
...
/cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/bin/GenTC -i Index_reference -l IU -1 <(gunzip -c xSimDat/WT_Mut_reference_sim_1.fasta.gz) -2 <(gunzip -c xSimDat/WT_Mut_reference_sim_2.fasta.gz) -p $CPUNUM -o $workdir

rm -r xSimDat

Rscript /cfs/klemming/projects/snic/snic2020-6-4/Meghana/MAX-binary-2.0.1/R/buildCRP.R in=Mutated_Combined_eqclass.txt out=$workdir/X_matrix.RData workdir=$workdir
