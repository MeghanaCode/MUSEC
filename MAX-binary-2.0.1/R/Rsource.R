...
mergeMut<-function(a,tt){
  ...
  #nghiavtr/31Dec2024: improve the speed

  a_merge=tapply(a, a$eqClass, rbind)

  a_merge=lapply(a_merge,function(b){
    wt=sapply(b$Transcript,function(x) strsplit(x,"_mut_")[[1]][1])
    wt=unique(wt)
    if (length(grep("mut",b$Transcript)) > 0){
      for (isoform in paste0(wt,'_mut')){       
...
getCRP <- function(rawmat,H_thres=0.025,ncore=1){
  ...
  {
  Xwt = X[,-index, drop=FALSE];Xwt=ccrpfun(Xwt, clim=50) #nghiavtr/31Dec2024
  Xmut= X[,index, drop=FALSE];Xmut=ccrpfun(Xmut, clim=50)
  X = cbind(Xwt, Xmut)
  CCRP[[j]] = X
  next;
  }
  if(length(index)==1&length(index_wt)>1)
  {
  Xwt = X[,-index, drop=FALSE];Xwt=ccrpfun(Xwt, clim=50)
  Xmut= X[,index, drop=FALSE]
  X1 = cbind(Xwt, X[,index, drop=FALSE])
