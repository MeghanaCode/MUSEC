...
library(data.table)
mut.list = fread(mut)
gList=strsplit(mut.list$Gene,";")
l=lengths(gList)
mut1=mut.list[which(l==1),]
pick=which(l>1)
if (length(pick)> 0){	
...
### nghiavtr/05April2024
#extract WT transcript cdna fasta
allwtTxFa=readDNAStringSet(ref)
allwtTxFa_names=names(allwtTxFa)
allwtTxFa_names=sapply(allwtTxFa_names,function(x) strsplit(x," ")[[1]][1])
names(allwtTxFa_names)=NULL
pick=which(allwtTxFa_names %in% mygene_txlist)
dupP=which(duplicated(allwtTxFa_names[pick]))
if (length(dupP)>0) pick=pick[-dupP]
txCDNAfa=allwtTxFa[pick]
writeXStringSet(txCDNAfa, "wild_type_cdna_sequences.fa")
...
### nghiavtr/20Nov2024 - start
## speed up the extraction time 
mySeq=NULL
grAll=NULL
for (i in 1: length(mygene_txlist)){
  tx=mygene_txlist[i]
  ...
  startR=mytx$EXONSTART
  endR=mytx$EXONEND
  gr = GRanges(seqnames=chrID, ranges=IRanges(start=startR, end=endR), txname=tx)
  grAll=c(grAll,gr)
}

grAllVector=do.call(c,grAll)
y=getSeq(fasta_genome, grAllVector)
mySeq=tapply(y,grAllVector$txname, function(x) paste0(x,collapse = ""))

m=match(mygene_txlist, names(mySeq))
mySeq=mySeq[m]
## nghiavtr/20Nov2024 -  end
