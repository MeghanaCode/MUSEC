...
#Rscript /nfs/NGS/Meghana/MAX-binary-2.0.1/R/MAX.R mut=$mut gtf=$gtf ref=$ref workdir=$workdir hg=$hg
...
#chmod +x /nfs/NGS/Meghana/MAX-binary-2.0.1/bin/*

cd $workdir
#mkdir WT_Mut_Ref
#mv WT_Mut_reference.fa WT_Mut_Ref
# Index
#/nfs/NGS/Meghana/MAX-binary-2.0.1/bin/TxIndexer -t WT_Mut_Ref/WT_Mut_reference.fa -o Index_reference -k $kmer --force  -p $CPUNUM 

## generate RNA-seq using polyester
mkdir xSimDat
#Rscript /nfs/NGS/Meghana/MAX-binary-2.0.1/R/genPolyesterSimulation.R WT_Mut_Ref/WT_Mut_reference.fa xSimDat
#/nfs/NGS/Meghana/MAX-binary-2.0.1/bin/GenTC -i Index_reference -l IU -1 xSimDat/sample_01_1.fasta -2 xSimDat/sample_01_2.fasta -p $CPUNUM -o $workdir
echo $workdir
### generate RNA-seq using a fast tool
/nfs/NGS/Meghana/MAX-binary-2.0.1/bin/simx_pair WT_Mut_Ref -o xSimDat -t $CPUNUM -d 20
#/nfs/NGS/Meghana/MAX-binary-2.0.1/bin/GenTC -i Index_reference -l IU -1 <(gunzip -c xSimDat/WT_Mut_reference_sim_1.fasta.gz) -2 <(gunzip -c xSimDat/WT_Mut_reference_sim_2.fasta.gz) -p $CPUNUM -o $workdir

#rm -r xSimDat

Rscript /nfs/NGS/Meghana/MAX-binary-2.0.1/R/buildCRP.R in=Mutated_Combined_eqclass.txt out=$workdir/X_matrix.RData workdir=$workdir
